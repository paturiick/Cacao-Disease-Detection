...
const currentIndex = ref(0); // Centralized index state for the carousel and grid
const flightPath = ref([]);  // Array to hold the drone's route
...
watch(() => telemetryState.gps_lat, (newLat) => {
  ...
  // Only add points if we have a valid GPS lock
...
const gpsData = computed(() => {
  ...
  if (telemetryState.battery >= 50) batColor = 'bg-green-500'; 
  else if (telemetryState.battery >= 20) batColor = 'bg-yellow-500'; 
...
<template>
  <div class="flex flex-col h-screen overflow-hidden bg-cover bg-center relative font-inter" style="background-image: url('https://images.unsplash.com/photo-1542319084-2a6c38210350?q=80&w=2574&auto=format&fit=crop');">
