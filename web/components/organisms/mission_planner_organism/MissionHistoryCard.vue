...
// Props and Emits returned to standard manual state
  ...
  <BaseCard class="h-full flex flex-col bg-white/95 backdrop-blur-sm relative">
    ...
    <div class="flex items-center gap-5">
      ...
      <button 
        ...
        :class="!isDrawingMode ? 'opacity-100 scale-105' : 'opacity-40 hover:opacity-70'"
      >
        <SectionHeader class="!mb-0" :class="{'!font-medium': isDrawingMode}">
          <template #icon>
    ...
    <div class="flex items-center gap-3">
       ...
       <button 
         v-if="queue.length > 0" 
         @click="$emit('clear-attempt')" 
         class="text-[10px] font-bold text-red-500 hover:text-red-700 underline uppercase tracking-tighter transition-colors"
